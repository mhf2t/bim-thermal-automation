...
from compliance import (
    CODES,
    CLIMATE_DATA,
    check_wall_compliance,
    check_roof_compliance,
    check_window_compliance,
    check_wwr_compliance,
    calculate_thermal_performance_index,
    get_tpi_grade,
    generate_insulation_recommendations,
    get_material_confidence_summary,
    compute_validation_stats,
    calculate_fabric_heat_loss,
    calculate_code_compliant_heat_loss,
)
...
:root {
  ...
  --sk:rgba(255,255,255,0.12);
  --sk2:rgba(255,255,255,0.18);
  --tx:rgba(255,255,255,0.92);
  --tx2:rgba(255,255,255,0.78);
  ...
  --pass:#34D399;
  --fail:#FB7185;
  --sh:0 10px 30px rgba(0,0,0,0.45);
  --sh2:0 18px 56px rgba(0,0,0,0.55);
  --rd:16px;
}
...
[data-testid="stSidebar"]{
  ...
  border-right:1px solid var(--sk2)!important;
...
.section-title{
  ...
  border-left:3px solid rgba(34,211,238,0.82);
  padding-left:10px;
...
CT = {
    ...
    "plot": "rgba(255,255,255,0.03)",
    "font": "rgba(255,255,255,0.88)",
    "muted": "rgba(255,255,255,0.65)",
    "grid": "rgba(255,255,255,0.09)",
    "pass": "rgba(52,211,153,0.85)",
    "fail": "rgba(251,113,133,0.50)",
    "warn": "#FBBF24",
    "accent": "#7C3AED",
    "cyan": "#22D3EE",
    "ff": "Manrope",
...
def chlayout(title="", h=400, l=20, r=20, t=48, b=20):
    return dict(
        ...
        xaxis=dict(
            gridcolor=CT["grid"],
            linecolor=CT["grid"],
            tickfont=dict(size=9, color=CT["muted"]),
        ),
        yaxis=dict(
            gridcolor=CT["grid"],
            linecolor=CT["grid"],
...
<div style="font-size:.78rem;color:rgba(255,255,255,0.65);line-height:1.55;">
  Upload any IFC file from Revit.<br>
  U-values calculated via <b>ISO 6946</b> from extracted material layer data.<br><br>
...
def compute_wall_results(filename, code_name, walls_json):
    ...
    walls_local = json.loads(walls_json)
    c = CODES[code_name]
    return [(w, check_wall_compliance(w["u_value"], c)) for w in walls_local if w["u_value"] is not None]
...
pass_types = sum(1 for w in unique_types if w["u_value"] and w["u_value"] <= code["wall_u_max"])
...
<div style="margin-top:.9rem;font-size:.74rem;color:rgba(255,255,255,0.62);">
  ...
  🟢 High: {conf['high_pct']}% &nbsp; 🟡 Med: {conf['medium_pct']}% &nbsp; 🔴 Low: {conf['low_pct']}%<br>
  <span style="font-size:.66rem;color:rgba(255,255,255,.44);"> Report in paper methodology section</span>
</div>
""",
        unsafe_allow_html=True,
    )
...
with c1:
    metric_card("Wall Instances", str(summary["total_external_walls"]), "elements in IFC")
with c2:
    metric_card("Unique Types", str(len(unique_types)), "distinct assemblies")
with c3:
    metric_card("Windows", str(summary["total_windows"]), "glazing units")
with c4:
    metric_card("Envelope Area", f"{summary['total_wall_area_m2']:.0f} m²", "total external wall")
with c5:
    wwr = summary["overall_wwr_pct"]
    metric_card(
        "Overall WWR",
        f"{wwr:.1f}%",
        f"limit: {code['wwr_max_pct']}%",
...
box-shadow:0 16px 44px rgba(0,0,0,0.35);backdrop-filter:blur(14px);">
  ...
  <div>
    ...
    <div style="font-size:.82rem;color:rgba(255,255,255,0.70);">
      All {len(unique_types)} unique assembly types pass {selected_code_name}
    </div>
  </div>
</div>
""",
...
box-shadow:0 16px 44px rgba(0,0,0,0.35);backdrop-filter:blur(14px);">
  ...
  <div>
    ...
    color:rgba(251,113,133,0.95);font-size:.90rem;">
      {fail_types} ASSEMBLY TYPE(S) FAILING — {pct}% NON-COMPLIANT
    </div>
    <div style="font-size:.82rem;color:rgba(255,255,255,0.70);">
      {pass_types} types passing · {fail_types} types failing · Standard: {selected_code_name}<br>
      <span style="font-size:.76rem;opacity:.75;">
        (Unique types: {len(unique_types)} · Total wall instances: {summary['total_external_walls']})
      </span>
    </div>
  </div>
</div>
    ...
    if not wall_results:
        ...
        for wall, result in sorted_walls:
            ...
            layers_str = " | ".join(
                ...
                for l in wall["layers"]
                if l.get("thickness_mm")
            )
            hover_texts.append(
                f"<b>{wall['name']}</b><br>"
        ...
        with col_right:
            ...
            fig_donut.add_annotation(
                ...
                y=0.5,
                showarrow=False,
                font=dict(size=14, color=CT["font"], family=CT["ff"]),
                align="center",
            )
            dl = chlayout("Assembly Type Pass / Fail", h=half_h, l=10, r=10, t=36, b=28)
            dl["showlegend"] = True
            dl["legend"] = dict(
                orientation="h", x=0.5, xanchor="center", y=-0.06, font=dict(size=10, color=CT["muted"])
            )
            ...
            bl["showlegend"] = True
            bl["legend"] = dict(
                orientation="h", x=0.5, xanchor="center", y=-0.10, font=dict(size=10, color=CT["muted"])
            )
...
<div class="info-box">
  ...
  <span style="font-size:.78rem;color:rgba(255,255,255,0.52);">
    ...
    Confidence = material name matching accuracy.
  </span>
</div>
""",
            unsafe_allow_html=True,
        ...
        for wall, result in sorted_walls:
            ...
            margin_txt = (
                f"✓ within limit by {abs(result['margin'])} W/m²K"
                if result["passed"]
            ...
            with st.expander(
                ...
                with ca:
                    ...
                    rows += [
                        {
                            "Layer": "Rsi",
                            "Material (IFC)": "Interior surface resistance",
                            "DB Match": "ISO 6946",
                            "Confidence": "STANDARD",
                            "Thickness (mm)": "—",
                            "λ (W/mK)": "—",
                        ...
                        {
                            "Layer": "Rso",
                            "Material (IFC)": "Exterior surface resistance",
                            "DB Match": "ISO 6946",
                            "Confidence": "STANDARD",
                            "Thickness (mm)": "—",
                            "λ (W/mK)": "—",
                ...
                with cb:
                    badge = '<span class="badge-pass">✅ PASS</span>' if result["passed"] else '<span class="badge-fail">❌ FAIL</span>'
                    st.markdown(
                        f"""
...
background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.11);">
  ...
  <div style="margin-top:.9rem;font-size:.78rem;color:{margin_col};">{margin_txt}</div>
</div>
""",
                        unsafe_allow_html=True,
                    )
...
# ══════════════════════════════════════════════════
with tab2:
    st.markdown('<div class="section-title">03 — R-VALUE ANALYSIS (ISO 6946)</div>', unsafe_allow_html=True)
    ...
    if not wall_results:
        ...
        if not unique_r:
            ...
            with colB:
                ...
                if threshold_r is not None:
                    r_pass = sum(1 for (_w, res) in unique_r if res.get("passed", False))
            ...
            st.download_button("⬇️ Export R-Value CSV", df_r.to_csv(index=False), "r_value_analysis.csv", "text/csv")

# ══════════════════════════════════════════════════
# TAB 3 — FACADE MAP
# ══════════════════════════════════════════════════
with tab3:
    st.markdown('<div class="section-title">03 — FACADE PERFORMANCE MAP</div>', unsafe_allow_html=True)
        ...
        if len(valid_f) >= 3:
            ...
            fig_r.add_trace(
                go.Scatterpolar(
                    ...
                    line=dict(color=CT["cyan"], width=2),
                    name="U-values",
                )
            )
            fig_r.add_trace(
            ...
            fig_r.update_layout(
                polar=dict(
                    radialaxis=dict(visible=True, color=CT["muted"], gridcolor=CT["grid"]),
                    angularaxis=dict(color=CT["font"]),
                ...
                title=dict(text="Area-Weighted U-Value by Facade", font=dict(size=12, color=CT["muted"], family=CT["ff"])),
...
<div class="metric-card">
  ...
  <div style="margin-top:.7rem;"><span class="{bc}">{'PASS' if passed else 'FAIL'}</span></div>
</div>
""",
                    unsafe_allow_html=True,
                )
...
# ══════════════════════════════════════════════════
with tab4:
    st.markdown('<div class="section-title">04 — WINDOW & GLAZING PERFORMANCE</div>', unsafe_allow_html=True)
    ...
    if not windows:
        ...
        for i, ori in enumerate(orientations):
            with wwr_cols[i]:
                ori_win_area = sum(w.get("area_m2", 0) for w in windows if w["orientation"] == ori and w.get("area_m2"))
                    ...
                    fig_g = go.Figure(
                        go.Indicator(
                            ...
                            gauge=dict(
                                ...
                                steps=[
                                    dict(range=[0, wwr_limit], color="rgba(52,211,153,0.10)"),
                                    dict(range=[wwr_limit, 100], color="rgba(251,113,133,0.08)"),
                                ],
                                threshold=dict(line=dict(color=CT["warn"], width=3), thickness=0.8, value=wwr_limit),
                            ),
                        )
                    )
                    fig_g.update_layout(
...
<div class="metric-card" style="text-align:center;">
  ...
  <div style="color:{CT['muted']};font-size:.80rem;padding:1rem;">No wall area data</div>
</div>
""",
                        unsafe_allow_html=True,
                    )

        st.markdown('<div class="section-title">WINDOW THERMAL PROPERTIES</div>', unsafe_allow_html=True)
            ...
            win_rows.append(
                {
                    ...
                    "SHGC Status": ("✅ PASS" if shgc and shgc <= code["shgc_max"] else ("❌ FAIL" if shgc else "⚠️ NO DATA")),
                }
            )
...
# ══════════════════════════════════════════════════
with tab5:
    st.markdown('<div class="section-title">05 — INSULATION IMPROVEMENT SCENARIOS</div>', unsafe_allow_html=True)
    ...
    if not recommendations:
        ...
        st.markdown(
            f"""
<div class="info-box">
  🔧 <b>{len(recommendations)} unique assembly type(s)</b> require upgrades to meet {selected_code_name}.<br>
...
background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.10);">
  ...
  <div style="margin-top:.6rem;font-size:.75rem;color:{CT['muted']};">Thermal gap: {pct_gap}%</div>
</div>
""",
                        unsafe_allow_html=True,
                    )
        ...
        if failing:
            st.markdown('<div class="section-title">COMPLIANCE IMPROVEMENT PATHWAY</div>', unsafe_allow_html=True)
            ...
            fig_wf = go.Figure(
                go.Waterfall(
                    ...
                    text=[f"{v:+.3f}" if m == "relative" else f"{v:.3f}" for v, m in zip(wf_v, wf_m)],
                    textposition="outside",
                    textfont=dict(family=CT["ff"], size=10, color=CT["font"]),
                )
            )
...
# ==========================================================
with tab6:
    st.markdown('<div class="section-title">05 — FABRIC HEAT LOSS ESTIMATION</div>', unsafe_allow_html=True)
    ...
    with k1:
        metric_card("Total Envelope Area", f"{hl.get('total_env_area',0):.0f} m²", "walls + roofs")
    with k2:
        metric_card(
            f"Peak {mode.capitalize()}",
    ...
    with k3:
        if compliant_total_W:
            ...
            metric_card("Potential Saving", f"{diff/1000:.2f} kW", "if fully code-compliant", CT["cyan"])
        else:
            metric_card("Code Scenario", "—", "No compliant baseline", CT["muted"])
    ...
    with col_left:
        st.markdown('<div class="section-title">HEAT LOSS BY WALL ASSEMBLY</div>', unsafe_allow_html=True)

        wall_loss_list = hl.get("wall_loss", [])
        if wall_loss_list:
            sorted_hl = sorted(wall_loss_list, key=lambda x: x.get(key_q, 0), reverse=True)
            ...
            layout = chlayout(f"Wall Assembly {mode.capitalize()} (W)", h=chart_height, l=5, r=60, t=40, b=20)
    ...
    with col_right:
        st.markdown('<div class="section-title">LOSS BREAKDOWN</div>', unsafe_allow_html=True)
        ...
        if wall_W > 0:
            labels.append("Walls"); values.append(wall_W); colors.append("rgba(251,113,133,0.8)")
        if roof_W > 0:
            labels.append("Roofs"); values.append(roof_W); colors.append("rgba(124,58,237,0.8)")
        if win_W > 0:
            labels.append("Windows"); values.append(win_W); colors.append("rgba(34,211,238,0.8)")

        if values:
            fig_pie = go.Figure(go.Pie(labels=labels, values=values, hole=0.55, marker=dict(colors=colors), textinfo="label+percent"))
            layout_pie = chlayout("Peak Contribution Share", h=260, l=10, r=10, t=40, b=10)
            fig_pie.update_layout(**layout_pie)
            st.plotly_chart(fig_pie, use_container_width=True)
        else:
            st.info("Breakdown not available.")

        st.markdown('<div class="section-title">ACTUAL vs. CODE-COMPLIANT</div>', unsafe_allow_html=True)
        ...
        fig_comp = go.Figure()
        fig_comp.add_trace(go.Bar(x=categories, y=actual_vals, name="Actual", marker_color=CT["fail"]))
        fig_comp.add_trace(go.Bar(x=categories, y=compliant_vals, name="If Code-Compliant", marker_color="rgba(52,211,153,0.7)"))

        layout_comp = chlayout(f"Actual vs Code-Compliant ({mode.capitalize()})", h=260, l=20, r=20, t=40, b=20)
...
# ══════════════════════════════════════════════════
with tab7:
    st.markdown('<div class="section-title">06 — VALIDATION PANEL (RESEARCH EVIDENCE)</div>', unsafe_allow_html=True)
...
<div class="info-box">
  ...
  <span style="font-size:.78rem;color:rgba(255,255,255,0.52);">
    ...
    Non-zero error reveals IFC extraction uncertainty — report this transparently.
  </span>
</div>
""",
        unsafe_allow_html=True,
    ...
    if not wall_results:
            ...
            val_rows.append(
                {
                    ...
                    "% Error": f"{pct:.4f}%",
                    "Agreement": ("✅ Excellent" if pct < 0.5 else "✅ Good" if pct < 2.0 else "⚠️ Review" if pct < 5.0 else "❌ Check"),
                }
            )
        ...
        st.download_button("⬇️ Export U-Value Validation CSV", df_val.to_csv(index=False), "u_value_validation_results.csv", "text/csv")
        ...
        if stats and "mape" in stats:
            ...
            with sv1:
                metric_card("MAPE", f"{stats['mape']:.4f}%", "Mean Abs % Error", CT["pass"] if stats["mape"] < 2 else CT["warn"])
            with sv2:
                metric_card("RMSE", f"{stats['rmse']:.6f}", "Root Mean Sq Error", CT["pass"] if stats["rmse"] < 0.01 else CT["warn"])
            with sv3:
                metric_card("Max Error", f"{stats['max_error_pct']:.4f}%", "Worst deviation", CT["pass"] if stats["max_error_pct"] < 2 else CT["warn"])
            with sv4:
                metric_card("R²", f"{stats['r_squared']:.6f}", "Coefficient of det.", CT["pass"] if stats["r_squared"] > 0.99 else CT["warn"])
            with sv5:
                verdict = "VALIDATED ✅" if stats.get("validated", False) else "REVIEW ⚠️"
                metric_card("Verdict", verdict, f"n = {stats.get('n', 0)} types", CT["pass"] if stats.get("validated", False) else CT["warn"])
            ...
            fig_sc.add_trace(
                go.Scatter(
                    ...
                    y=all_t,
                    mode="markers+text",
                    marker=dict(color=CT["cyan"], size=11, opacity=0.88, line=dict(color="rgba(255,255,255,0.22)", width=1)),
                    text=names_l,
            ...
            sc_lay = chlayout(f"Manual vs Tool — U-Value Validation (R² = {stats['r_squared']:.6f})", h=380, l=55, r=20, t=48, b=30)
...
<div class="info-box">
  ...
  "The automated IFC-to-U-value pipeline was validated against independent manual ISO 6946 calculations across
  <b>{stats['n']}</b> unique wall assembly types extracted from the case study IFC file.
  The framework achieved MAPE <b>{stats['mape']:.4f}%</b>, RMSE <b>{stats['rmse']:.6f} W/m²K</b>, and R² <b>{stats['r_squared']:.6f}</b>,
  confirming mathematical accuracy and reproducibility."
</div>
""",
                unsafe_allow_html=True,
            )
...
<div class="info-box">
  ...
  Compare tool outputs against independent spreadsheet calculations using the same extracted U-values, areas, and the code-defined ΔT.<br>
  <span style="font-size:.78rem;color:rgba(255,255,255,0.52);">
...
""",
    ...
    if delta_heat <= 0 and delta_cool <= 0:
        ...
        with lf1:
            man_walls = st.number_input("Manual Walls (W)", min_value=0.0, value=float(tool_walls), step=10.0, key=f"man_walls_q_{mode_v}")
        with lf2:
            man_roofs = st.number_input("Manual Roofs (W)", min_value=0.0, value=float(tool_roofs), step=10.0, key=f"man_roofs_q_{mode_v}")
        with lf3:
            man_wins = st.number_input("Manual Windows (W)", min_value=0.0, value=float(tool_wins), step=10.0, key=f"man_wins_q_{mode_v}")
        with lf4:
            man_total = st.number_input("Manual TOTAL (W)", min_value=0.0, value=float(tool_total), step=10.0, key=f"man_total_q_{mode_v}")
        ...
        st.download_button("⬇️ Export Fabric Load Validation CSV", df_load.to_csv(index=False), f"fabric_load_validation_{mode_v}.csv", "text/csv")
        ...
        if load_stats and "mape" in load_stats:
            ...
            with q1:
                metric_card("MAPE", f"{load_stats['mape']:.4f}%", "Mean Abs % Error", CT["pass"] if load_stats["mape"] < 2 else CT["warn"])
            with q2:
                metric_card("RMSE", f"{load_stats['rmse']:.3f}", "W (Root Mean Sq Error)", CT["pass"] if load_stats["rmse"] < 50 else CT["warn"])
            with q3:
                metric_card("Max Error", f"{load_stats['max_error_pct']:.4f}%", "Worst deviation", CT["pass"] if load_stats["max_error_pct"] < 2 else CT["warn"])
            with q4:
                metric_card("R²", f"{load_stats['r_squared']:.6f}", "Coefficient of det.", CT["pass"] if load_stats["r_squared"] > 0.99 else CT["warn"])
            ...
            fig_q.add_trace(
                go.Scatter(
                    ...
                    y=yt,
                    mode="markers+text",
                    marker=dict(color=CT["cyan"], size=12, opacity=0.90, line=dict(color="rgba(255,255,255,0.22)", width=1)),
                    text=lbl,
                    textposition="top center",
            ...
            q_lay = chlayout(f"Manual vs Tool — Fabric {mode_v.capitalize()} Load (R² = {load_stats['r_squared']:.6f})", h=360, l=55, r=20, t=48, b=30)
...
<div class="info-box">
  ...
  "The envelope fabric load aggregation (<code>Σ(U·A·ΔT)</code>) was validated against independent spreadsheet calculations
  at the component level (walls, roofs, windows) and at the total envelope level under the selected {selected_code_name} climate
  (ΔT = <b>{deltaT:.1f} K</b>).
  The framework achieved MAPE <b>{load_stats['mape']:.4f}%</b>, RMSE <b>{load_stats['rmse']:.3f} W</b>, and R² <b>{load_stats['r_squared']:.6f}</b>,
  confirming computational correctness of the steady-state conductive load estimation."
</div>
""",
                unsafe_allow_html=True,
            )
...
# ══════════════════════════════════════════════════
with tab8:
    st.markdown('<div class="section-title">07 — RAW EXTRACTED DATA & TRANSPARENCY REPORT</div>', unsafe_allow_html=True)
...
<div class="info-box">
  ...
  <span style="font-size:.76rem;color:rgba(255,255,255,0.48);">
    Report this breakdown in your paper's methodology/limitation section for transparency.
  </span>
</div>
""",
        unsafe_allow_html=True,
            ...
            rows = [
                {
                    ...
                    "Orientation": w["orientation"],
                    "Area (m²)": w["area_m2"],
                    "U-Value (W/m²K)": w["u_value"],
                    "Thickness (mm)": w["total_thickness_mm"],
                    "Layers": w["layer_count"],
                    "Compliance": check_wall_compliance(w["u_value"], code)["status"] if w["u_value"] else "—",
                }
                for w in walls
            ]
            ...
            rows = [
                {
                    ...
                    "Window": w["name"],
                    "Orientation": w["orientation"],
                    "Width (m)": w.get("width_m"),
                    "Height (m)": w.get("height_m"),
                    "Area (m²)": w.get("area_m2"),
                    "U-Value": w.get("u_value"),
                    "SHGC": w.get("shgc"),
                }
                for w in windows
            ]
            ...
            rows = [
                {
                    "IFC GlobalId": r["id"],
                    "Roof Name": r["name"],
                    "U-Value (W/m²K)": r["u_value"],
                    "Area (m²)": r["area_m2"],
                    "Layers": r["layer_count"],
                    "Compliance": check_roof_compliance(r["u_value"], code)["status"] if r["u_value"] else "—",
                }
                for r in roofs
            ]
            st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
