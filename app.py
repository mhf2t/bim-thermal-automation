...
# ══════════════════════════════════════════════════
# TAB 7 — VALIDATION (SAFE / UNCRASHABLE)
# ══════════════════════════════════════════════════
with tab7:
    try:
        ...
        if not wall_results:
            ...
            # Deduplicate by wall type name
            unique_results = []
            seen = set()
            for wall, result in wall_results:
                nm = wall.get("name", "Unnamed")
                if nm not in seen:
                    seen.add(nm)
                    unique_results.append((wall, result))

            if len(unique_results) == 0:
                st.warning("No unique wall types found for validation (unique_results is empty).")
            else:
                ...
                # ✅ GUARDED columns count
                n_cols = max(1, min(3, len(unique_results)))
                cols_v = st.columns(n_cols)

                for idx, (wall, result) in enumerate(unique_results):
                    wall_name = wall.get("name", "Unnamed")
                    wall_u = float(wall.get("u_value") or 0.0)

                    with cols_v[idx % n_cols]:
                        manual = st.number_input(
                            f"{wall_name[:30]}",
                            min_value=0.001, max_value=15.0,
                            value=float(wall_u if wall_u > 0 else 0.5),
                            step=0.001, format="%.4f",
                            # ✅ SAFE KEY: uses index + name (prevents duplicates)
                            key=f"v_{idx}_{wall_name[:12]}",
                            help=f"Tool calculated: {wall_u:.4f} W/m²K"
                        )

                    tool_val = wall_u
                    diff = abs(tool_val - manual)
                    pct = (diff / manual * 100) if manual > 0 else 0.0

                    val_pairs.append({"manual": float(manual), "tool": float(tool_val)})
                    val_rows.append({
                        "Wall Assembly": wall_name,
                ...
                try:
                    ...
                    stats = None
                    st.warning(f"compute_validation_stats failed: {e}")
                ...
                if stats and "mape" in stats:
                    ...
                    if len(all_m) >= 1:
                        ...
                        fig_sc.add_trace(go.Scatter(
                            x=all_m, y=all_t, mode="markers",
                            marker=dict(color=CT["cyan"], size=11, opacity=0.88),
                            name="Wall assemblies",
                            hovertemplate="Manual:%{x:.4f}<br>Tool:%{y:.4f}<extra></extra>"
                        ))
        ...
        # ======================================================
        # B) FABRIC LOAD VALIDATION (optional)
        # ======================================================
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        Envelope conductive load uses <code>Q = Σ(U · A · ΔT)</code> (fabric conduction only).<br>
        </div>
        """, unsafe_allow_html=True)
        ...
        if delta_heat <= 0 and delta_cool <= 0:
            st.info("No ΔT defined for this code/climate → skipping fabric load validation.")
        else:
            mode_v = "heating" if delta_heat > 0 else "cooling"
            deltaT = delta_heat if mode_v == "heating" else delta_cool
            ...
            with a1:
                man_walls = st.number_input("Manual Walls (W)", min_value=0.0, value=float(tool_walls), step=10.0, key=f"manW_{mode_v}")
            with a2:
                man_roofs = st.number_input("Manual Roofs (W)", min_value=0.0, value=float(tool_roofs), step=10.0, key=f"manR_{mode_v}")
            with a3:
                man_wins  = st.number_input("Manual Windows (W)", min_value=0.0, value=float(tool_wins), step=10.0, key=f"manG_{mode_v}")
            with a4:
                man_total = st.number_input("Manual TOTAL (W)", min_value=0.0, value=float(tool_total), step=10.0, key=f"manT_{mode_v}")

            df_load = pd.DataFrame([
                {"Component":"Walls","Manual (W)":man_walls,"Tool (W)":tool_walls},
                {"Component":"Roofs","Manual (W)":man_roofs,"Tool (W)":tool_roofs},
                {"Component":"Windows","Manual (W)":man_wins,"Tool (W)":tool_wins},
